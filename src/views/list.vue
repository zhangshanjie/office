...
export default {
  ...
  methods: {
    ...
    getDataList() {
      this.loading = true;
      this.$refust("/appgamehonor/getHonorTopicPage", this.searchForm).then(
        (res) => {
          this.loading = false;
          if (res.success) {
            this.total = res.result.total;
    ...
    Submit() {
      ...
      this.$http("/appgamehonor/addHonorGloryInfo", this.advertisings).then(
        (res) => {
          if (res.success) {
    ...
    del(id) {
      this.$confirm("此操作将永久删除该列表, 是否继续?", "提示", {
        ...
        .then(() => {
          ...
          this.$http("/appgamehonor/delHonorTopic", params).then((res) => {
            if (res.success) {
              this.getDataList();
            }
          });
